...
dataset_path = "./person_dataset/"
pedestrian_images_dataset_dir = os.listdir(dataset_path + "pedestrian")
non_pedestrian_images_dataset_dir = os.listdir(dataset_path + "non_pedestrian")
pedestrian_detection_train_dataset = []
one_hot_encoded_array = []
count = 0
for image_file_name in non_pedestrian_images_dataset_dir:
    image = cv2.imread(dataset_path + "non_pedestrian/" + image_file_name)
    new_image = preprocess_image(image)
    pedestrian_detection_train_dataset.append(new_image)
    one_hot_encoded_array.append([0, 1])
non_pedestrian_image_count = len(one_hot_encoded_array)
for image_file_name in pedestrian_images_dataset_dir:
    count += 1
    if count >  non_pedestrian_image_count:
        break
    image = cv2.imread(dataset_path + "pedestrian/" + image_file_name)
    new_image = preprocess_image(image)
    pedestrian_detection_train_dataset.append(new_image)
    one_hot_encoded_array.append([1, 0])


for i in range(0, len(pedestrian_detection_train_dataset)):
    cv2.imshow("image", pedestrian_detection_train_dataset[i])
    print(one_hot_encoded_array[i])
    cv2.waitKey(3)

model.fit(x=np.array(pedestrian_detection_train_dataset), y=np.array(one_hot_encoded_array), epochs=7)

print("Prediction for first image")
print(model.predict(np.expand_dims(pedestrian_detection_train_dataset[0], 0)))
print("Prediction for second image")
print(model.predict(np.expand_dims(pedestrian_detection_train_dataset[len(pedestrian_detection_train_dataset) - 5], 0)))
...
while True:
    ...
    if prediction[0][0] < prediction[0][1]:
        print("Image is detected as not person")
    else:
        print("Image is detected as person")
